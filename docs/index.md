...
| 简述 | Logo图 | 说明 |
| --- | --- | --- |
| **zxtool配置文件**||交互式生成和管理全局 zxtool.toml 配置文件
| **硬件基础信息**| |获取计算机的 CPU、GPU、内存、硬盘等相关信息
| **Git项目配置**|<img src = "./assets/images/screwdriver.webp" style="height:32px;"/>|管理 Git 仓库的 user.name 和 user.email 配置,支持自动填充
| **Markdown转PDF**|<img src = "./assets/images/pliers.webp" style="height:32px;"/>|将单个 Markdown 或文档目录转换为 PDF,支持 Mermaid 图形
| **图片处理**|<img src = "./assets/images/hammer.webp" style="height:32px;"/>|调整图片尺寸或压缩文件大小
| **MkDocs项目构建**|<img src = "./assets/images/wrench.webp" style="height:32px;"/>|批量构建和发布 MkDocs 文档站点,支持开发服务器预览
| **EPUB转Markdown**||将Epub文件转换成Markdown目录
| **TOTP解析**||解析 TOTP 双因素认证密钥
| **在线视频下载**||基于 yt-dlp 下载在线视频
| **目录备份拷贝**||备份一个目录到目标目录
| **HTTP静态文件服务**||快速启动本地静态文件 HTTP 服务,用于预览和临时共享目录
| **HTTP视频播放**||快速启动本地HTTP 服务,用于播放本地视频
| **Nginx站点配置**||根据配置自动生成和管理 Nginx 站点配置,支持 HTTPS 和批量发布
| **Let's Encrypt证书**||通过 ACME v2 协议获取免费证书,支持 DNS-01 自动验证和续签
| **自签SSL证书**||生成单域名或泛域名自签证书,支持 SAN,方便开发调试
